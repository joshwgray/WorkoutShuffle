...
public class CreateWorkout : IRequest<CreateWorkoutDto>
{
    /// <summary>
    ...
    public class Handler : IRequestHandler<CreateWorkout, CreateWorkoutDto>
    {
        private readonly ILogger<CreateWorkout> _logger;

        public Handler(ILogger<CreateWorkout> logger)
        {
            _logger = logger;
        }

        public Task<CreateWorkoutDto> Handle(CreateWorkout request, CancellationToken cancellationToken)
        {
            var r = new Random();
            ...
            switch (request.WorkoutType)
            {
                case WorkoutTypes.Endurance:
                    CreateWorkout(r, startTime, request, workout);
                    break;
                case WorkoutTypes.Threshold:
                    GenerateWorkoutRecursive(r, startTime, request, workout);
                    break;
        ...
        private static Task<CreateWorkoutDto> GenerateWorkoutRecursive(Random r, double startTime,
            CreateWorkout request, CreateWorkoutDto workout)
        {
            TimeInZone = 0;
            // create the workout
            CreateWorkout(r, startTime, request, workout);
            ...
            {
                return GenerateWorkoutRecursive(r, startTime, request, workout);
            }
            else
            {
                return Task.FromResult(workout);
        ...
        private static void CreateWorkout(Random r, double startTime, CreateWorkout request, CreateWorkoutDto workout)
        {
            SetCourseHeaderDetails(request.Ftp, request.WorkoutType, workout);
            SetCourseDataDetails(request.Ftp, startTime, r, request.WorkoutType, workout);
        }

        private static void SetCourseHeaderDetails(int ftp, WorkoutTypes? workoutType, CreateWorkoutDto workout)
        {
            ...
            workout.FileName = $"FILE NAME = Random_{workoutType}.erg";
            workout.Ftp = $"FTP = {ftp}";
            workout.MinutesWatts = "MINUTES WATTS";
            workout.EndCourseHeader = "[END COURSE HEADER]";
        }

        private static void SetCourseDataDetails(int ftp, double startTime, Random r, WorkoutTypes? workoutType,
        ...
        private static double SetWarmupValues(int ftp, double startTime, double warmupPercentFtp, double warmupDuration,
            CreateWorkoutDto workout)
        {
        ...
        private static double AddRepeats(int ftp, double warmupEnd, Random r, WorkoutTypes? workoutType,
            CreateWorkoutDto workout)
        {
